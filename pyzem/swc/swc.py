...
def compute_platform_area(r1, r2, h):
    return (r1 + r1) * h * math.pi

def compute_volume(r1,r2,h):
    return (r1*r1+r1*r1+r1*r1)*h*math.pi/3
...
class SwcNode(NodeMixin):
    ...
    def distance(self, tn):
        ...
        return 0.0
    def _surface(self, tn):
        """
        """
        if tn and self.is_regular() and tn.is_regular():
            height = self.distance(tn)
            surface = compute_platform_area(self._radius,tn._radius,height)
            return surface
        return 0.0
    def _volume(self, tn):
        """
        ...
        if tn and self.is_regular() and tn.is_regular():
            height = self.distance(tn)
            volume = compute_volume(self._radius,tn._radius,height)
            return volume
        return 0.0
...
#                     print(data)
    ...
    def delete_minpath(self,persent):
        ...
        while mindist/maxdist<persent:
            niter = iterators.PreOrderIter(self._root)
            for tn in niter:
                if tn.is_regular() and len(tn.children) ==0 and compute_pathdist(tn) == mindist:
                    tn.cutoff()
                    count +=1
                    maxdist = self.pathdistance()[1]
                    mindist = self.pathdistance()[2]
...
#pid = parent_id, pidl = parent_id_before, pidll= parent_id_before_before ~_~
        count = 0
        pidbb = 0
        pidb = 0
